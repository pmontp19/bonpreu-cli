...
2. ~~**`orders show` endpoint.**~~ **RESOLVED 2026-06-30.** `GET /api/order/v6/orders/<orderId>/decorated` → `{entities:{order,product}, result:[lineItems]}` (normalizr shape; denormalize via `entities.product`).
...
4. **WAF/cookie expiry UX** — detect `403`/expired `VISITORID` and emit a clear "re-run `import-har`" instruction.
