...
3. **`ecom-request-source-version` drift** — **DEFERRED 2026-07-01 (decision: pin, no auto-refresh in v1).** It's the frontend build-hash string (`2.0.0-<date>-<commit>`) that Bonpreu bumps on every web redeploy (timestamp is second-granular). The CLI pins it from the HAR at import (`client/har.go`) and sends it verbatim; it is never refreshed. **Risk:** if the backend ever rejects a stale value, every command fails until the user re-runs `import-har`. **Not observed to date** — the pinned value keeps working across redeploys, so this stays a latent risk, not an active bug. **Deferred fix (v2):** on a rejection (or at startup), scrape the current value from the live homepage JS/`__INITIAL_STATE__` and self-update the session (cf. ivorpad's Algolia app-id self-refresh). Revisit only if a stale-version rejection is actually seen in the wild.
