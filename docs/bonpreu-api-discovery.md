...
- **There is NO client-side access/refresh token.** Auth = cookies. → Confirms the CLI must use HAR/cookie import; no headless renewal possible. Re-import when `VISITORID` (~3 months) expires.
